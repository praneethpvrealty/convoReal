...
- Apply an additive migration (new tables, functions or indexes, `CREATE OR REPLACE`, `IF NOT EXISTS`) as soon as the branch is pushed, so the Vercel preview and the merged `main` both find the schema they expect. Hold a destructive or ordering-dependent one (drops, column type changes, data-rewriting backfills) until the PR is green and merged. Say which of the two applied.
