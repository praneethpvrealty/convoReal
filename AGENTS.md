...
- **A property share is recorded only through the ledger writer, never by writing `property_shares` yourself.** Server code calls `logPropertyShare` / `logListingsSent` in `src/lib/whatsapp/share-property-send.ts`; the browser and the mobile app post to `POST /api/properties/share-log`. Recording a share and capturing the contact×property pair on the journey are one step there, and the ledger's `AFTER INSERT` trigger (`journey_capture_from_share`) makes the same capture for any row that reaches the table another way, reading `journey_visible` off the row — so a share cannot skip the journey whichever path wrote it. Pass `journeyVisible: true` only for a share the agent composed for one contact; every broadcast or bot batch waits hidden in the Captured tray (JRN-005, JRN-009). From mid-August to late September 2026 every share surface but one wrote the ledger and skipped the journey; this rule and the trigger exist so that cannot recur.
...
CI (`.github/workflows/ci.yml`) is tiered. A `changes` job classifies the diff, then `lint`, `typecheck` and `test` run in parallel whenever web paths changed, and the `mobile` job runs only when `mobile/**` changed. `build` runs on `merge_group` and `push: main`, and on a pull request **only when the diff adds or renames a route** (`newRoutes` in the `changes` job: any added/renamed `route.ts`, `page.tsx`, `layout.tsx`, `default.tsx`, `loading|error|not-found.tsx`, plus `next.config.ts` and `ci.yml` itself). An ordinary PR skips it, because Vercel already builds every push and `typecheck` covers the same type errors; a *new* route is the case neither of those catches, since its failure surfaces at build time and nowhere earlier. `.next/cache` is restored between runs. Expect Build to run on your PR whenever you add a route — that is the gate working, not a misconfiguration. `ci` is an `if: always()` gate job and is the single status check to mark required; do not require the individual jobs, since a skipped job never reports. Run the same commands locally before pushing.
...
| WhatsApp     | `conversations`, `messages`, `message_reactions`, `message_templates`, `whatsapp_config`, `whatsapp_number_profiles`, `whatsapp_number_change_notices`, `whatsapp_retired_number_replies`, `whatsapp_meta_flows`, `whatsapp_meta_flow_sessions`, `whatsapp_reply_bridges`, `routing_rules` |
