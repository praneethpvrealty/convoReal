...
- [ ] **WhatsApp connection + saved numbers (mobile)**: the Settings → WhatsApp connection screen, and with it the "Saved numbers" card that switches the live number without a PIN (`src/components/settings/whatsapp-number-profiles.tsx`) and the "Number changed" card that notifies recent contacts (`whatsapp-number-change-card.tsx`, `GET /api/whatsapp/number-change`, `POST /api/whatsapp/number-change/notify`), ships on web only, as does the per-number auto-reply switch on the same card (`auto_reply_enabled` / `auto_reply_message` via `PATCH /api/whatsapp/config/profiles`). The precursor notice and the retired-number auto-reply themselves run in the dispatcher and the webhook handler, and so reach every surface. The logic is fully mobile-reachable — `GET`/`PATCH`/`DELETE /api/whatsapp/config/profiles` and `POST /api/whatsapp/config/profiles/[id]/activate` are bearer-auth routes over `src/lib/whatsapp/number-profiles.ts` — so closing the gap is one settings screen consuming them. Same root cause, same deferral.
...
- [ ] **Invoice Settings UI (mobile)**: the issuer block an invoice prints — legal name, RERA, PAN, GSTIN, default GST rate and posture, bank details, signature — is editable on web only (Settings → Invoicing). Same root cause, same deferral. The per-invoice **Charge GST** toggle itself is at full parity, so a mobile agent can raise an invoice with or without tax; what they cannot do is set up the issuer GSTIN or default rate that a taxed invoice prints, which an admin does once on web. `GET`/`PUT /api/invoice-settings` is already bearer-auth, so closing the gap is one screen consuming it. (`src/components/settings/invoice-settings-card.tsx`)
...
### Deferred: parties, what stages 1–3 left out

_A party is several people on one requirement — a couple buying together, or two or three people from one firm (migration 288, `src/lib/contacts/parties.ts`). Stage 1 shipped the model, manual linking and dedup of the two WhatsApp cards. Stage 2 made `getMatchingContacts` collapse to one row per deal when a caller passes a party index, and wired it into Match Radar events and the buyer digest. Stage 3 added suggestions (`src/lib/contacts/party-suggestions.ts`, migration 289) from signals already in the book. What is still open:_
...
- [x] **Phase 1 — internal transaction control**: Journey → Deal conversion with provenance, `/deals` index and six-tab detail on web and mobile, internal financials with the Token Safe rule, immutable `deal_events` (database-enforced), milestone checklist separate from pipeline stages, deal-linked tasks, document status/supersession/expiry, deal bundles. Migrations `20260918010000` (additive) and `20260918010100` (stage-change trigger, held to merge).
- [x] **Phase 2 — controlled collaboration**: `deal_stakeholders` (buyer, seller, advocate, banker, broker, witness; buyer/seller/internal side), per-stakeholder tokenised links (`deal_share_links`, SHA-256 stored, 24h/7d/30d expiry, revoke, view count, hashed access log), the one visibility resolver behind every `/api/public/deal-share` route (guarded by `external-view.test.ts`), visibility on events, milestones and documents, per-token email OTP with a signed unlock (never a Supabase Auth user), watermarked photo access, the public portal at `/deal/[token]`, Stakeholders tab and visibility pickers on web and mobile. Migrations `20260918030000` (additive) and `20260918030100` (held to merge). Stated gaps: **PDF watermarking** needs a PDF library the dependency tree does not have (a dependency decision, not a feature slip); **WhatsApp OTP** waits for Phase 3's template work, so a code-protected link needs the stakeholder's email; **"selected people" visibility** is not modelled — a link's side is the unit.
- [x] **Phase 3 — publishing updates**: `deal_updates` (insert-only, database-enforced) freezes a headline, message and the quoted milestones and entries the audience may already see; a correction is a new update naming `supersedes_update_id`. Per-recipient delivery on `deal_update_recipients`: the business number sends free-form inside the 24-hour window and the approved `purchase_progress_notice` template to a buyer outside it (the link follows their tap), personal WhatsApp is a `wa.me` handoff the app never sends, and sent / opened / acknowledged are three separate timestamps. Preview per recipient, Updates tab on web and mobile, Acknowledge on the portal. Migrations `20260918050000` (additive) and `20260918050100` (held to merge). Stated gaps: **seller-side notices outside the window** have no honest template and are handed over from the agent's phone (no new template is submitted — §2.7); **WhatsApp OTP** still needs an AUTHENTICATION template; **PDF watermarking** and **"selected people" visibility** remain as recorded above.
- [x] **Phase 4 — bundles on screen and date reminders**: the Bundle chip and picker on the record header (web and mobile) over the existing `POST /api/deal-groups`; `deal_deadlines` / `deal_deadlines_for_account` (migration `20260924103000`, additive) as the one rule for a milestone target date or expected close date that is due, read by Focus on both surfaces, the web Today page and the agent task digest (`src/lib/deals/deadlines.ts`). - [x] **Phase 5 — payment schedule**: `deal_payment_tranches` (migration `20260924140000`, additive) with `/api/deals/[id]/tranches`, server-summed totals, a Payment schedule block under the financials on web and mobile, and unpaid tranche due dates on the deadline watch (migration `20260924140100`, held to merge). `src/lib/deals/tranches.ts`. The Records index now also shows and sorts by the expected close date (migration `20260924150000`, held to merge).
