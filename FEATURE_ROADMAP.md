...
### Transaction Workspace: Phase 1 shipped, Phases 2–3 open
...
- [ ] **Phase 3 — publishing updates**: compose an update from selected milestones and events, preview per recipient, publish as a durable snapshot on the timeline (a correction is a new update referencing the previous one), send a fixed-format link notice through approved WhatsApp templates (`purchase_progress_notice` for suitable buyer updates), personal WhatsApp as an agent-initiated `wa.me` handoff, and sent / opened / acknowledged recorded separately. Touches template delivery, so it ships standalone, never in a validation train.
