...
- [ ] **WhatsApp connection + saved numbers (mobile)**: the Settings → WhatsApp connection screen, and with it the "Saved numbers" card that switches the live number without a PIN (`src/components/settings/whatsapp-number-profiles.tsx`), ships on web only. The logic is fully mobile-reachable — `GET`/`PATCH`/`DELETE /api/whatsapp/config/profiles` and `POST /api/whatsapp/config/profiles/[id]/activate` are bearer-auth routes over `src/lib/whatsapp/number-profiles.ts` — so closing the gap is one settings screen consuming them. Same root cause, same deferral.
