...
- [ ] **Qualification call campaigns**: broadcast-style outbound calls to a listing's enquirers ("is ₹14.7 Cr within your budget?") — recipients seeded from `contact_property_inquiries`, editable per campaign; stated budget/areas written back over the inferred ones and the contact retagged, so matching can propose alternatives. `voice_campaigns` + `voice_campaign_recipients`, calling-hours + retry + opt-out guardrails, credit-gated. Not started.
