...
### Transaction Workspace: Phases 1–4 shipped
...
- [x] **Phase 4 — bundles on screen and date reminders**: the Bundle chip and picker on the record header (web and mobile) over the existing `POST /api/deal-groups`; `deal_deadlines` / `deal_deadlines_for_account` (migration `20260924103000`, additive) as the one rule for a milestone target date or expected close date that is due, read by Focus on both surfaces, the web Today page and the agent task digest (`src/lib/deals/deadlines.ts`). Stated gaps: **a payment schedule** (per-tranche amount, due, received, instrument) is still one token entry plus free text, a product decision recorded in `docs/transaction-workspace.md`; **expected close on the Records index** is not shown as a column.
