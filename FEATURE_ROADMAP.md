...
- [ ] **Qualification call campaigns**: broadcast-style outbound calls to a listing's enquirers ("is ₹14.7 Cr within your budget?") — recipients seeded from `contact_property_inquiries`, editable per campaign; stated budget/areas written back over the inferred ones and the contact retagged, so matching can propose alternatives. Partial: server side shipped (migration 270, `src/app/api/voice-campaigns/`, `/api/cron/voice-campaigns` dispatcher, `src/lib/voice/` with a provider-pluggable dialer — `VOICE_CALL_PROVIDER=custom` routes calls to any HTTP bridge as a Sarvam outage/pricing hedge — and webhook writeback with opt-out + Qualified/Budget Mismatch tags) and the web UI ships as the "Voice Calls" tab on `/broadcasts` (`voice-campaigns-content.tsx`: list, create with enquiry seeding, start/pause, recipient add/remove, qualification results). A §2.8 gap, stated rather than silent: the mobile campaigns screen is not built — same root cause as the other account-administration gaps (no mobile surface of this kind yet); the API is mobile-ready (bearer-token auth). Per-call credit gating also not built.
