...
### Deferred: parties, stages 2 and 3

_A party is several people on one requirement — a couple buying together, or two or three people from one firm (migration 288, `src/lib/contacts/parties.ts`). Stage 1 shipped the model, manual linking, and dedup of the two WhatsApp cards: the follow-up radar and the closing card now emit one card per deal, address the party's chosen member, and measure silence across the party, so a reply from the wife resets the husband's clock. What it deliberately does not do yet:_

- [ ] **Matching and digest dedup (stage 2)**: `src/lib/matching.ts` still runs per contact, so a couple with the same requirement produces two match sets, two Match Radar events and two lines in the buyer digest. The party is the unit that should be matched. Until then, the workaround is `requirement_active = false` on the non-primary member, which parks their requirement (migrations 194/197) while keeping the record.
- [ ] **Link suggestions (stage 3)**: linking is manual. The signals to suggest it are already in the schema — same `last_inquired_property_id`, journey items on the same property, shared `company` or email domain, shared surname plus matching requirement — and the duplicate-suggestion UI (`src/components/contacts/duplicates-panel.tsx`) is the right shape to reuse, with "Link as one party" as a distinct verb from "Merge as duplicate". The two must never be confused: merge is for one person recorded twice and collapses the rows; linking keeps every contact whole.
- [ ] **Party editing on mobile (§2.8)**: the mobile contact screen shows who a contact buys with, read-only (`BuysWithRow` in `mobile/app/(app)/contact/[id].tsx`), and every behavioural rule is server-side so both surfaces already dedup identically. Linking and unlinking ship on web only (`src/components/contacts/party-panel.tsx`); closing the gap means a native picker over the same `/api/contacts/[id]/party` route, which already accepts mobile bearer tokens.
