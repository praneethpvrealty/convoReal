...
- [ ] **Phase 2 — controlled collaboration**: `deal_stakeholders` (buyer, seller, advocate, banker, broker, witness; side assignment), per-recipient tokenised transaction links with expiry, revoke, access log and view count, one visibility resolver on every external endpoint (`src/lib/deals/visibility.ts` already holds the rule and the cross-seller bundle test), optional per-token OTP for sensitive documents (never a Supabase Auth user), watermarked external document access via `src/lib/storage/watermark.ts`.
