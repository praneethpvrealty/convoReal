  ...
  RETURN (
    ...
    grouped AS (
      SELECT
        ...
        (array_agg(stages.id ORDER BY stages.position DESC) FILTER (
          WHERE NOT items.hidden AND items.status = 'active'
        ))[1] AS live_stage_id,
...
COMMENT ON FUNCTION public.journey_overview_groups(UUID, TEXT) IS
  'Account-scoped buyer/property Journey summaries. A journey sits at the furthest stage of its live items, and also at the account''s lost stage (lost_stage_id) while any item is dropped; with nothing live it sits at the lost stage alone. Contact identity fields preserve the caller-visible contacts_select team and assignment scope.';
