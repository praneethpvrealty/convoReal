-- ============================================================
-- 229_conversation_dedupe_unique.sql
...
DROP TABLE IF EXISTS conversation_merge_map_229;
CREATE TABLE conversation_merge_map_229 AS
SELECT
...
DELETE FROM conversation_merge_map_229 WHERE duplicate_id = survivor_id;
...
FROM conversation_merge_map_229 mm
...
WITH groups AS (
  ...
  FROM conversations c
  JOIN (
    SELECT survivor_id, duplicate_id FROM conversation_merge_map_229
    UNION ALL
    SELECT DISTINCT survivor_id, survivor_id FROM conversation_merge_map_229
...
USING conversation_merge_map_229 mm
WHERE c.id = mm.duplicate_id;

DROP TABLE conversation_merge_map_229;
