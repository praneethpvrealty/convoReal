...
8. **Native WhatsApp Flows — Buyer Preference Intake (Migration 125)**:
   ...
   - Triggers: buyer texts "update my preferences" (or `update_preferences` button) → `webhook-handler.ts` sends the flow; agents can send via `POST /api/whatsapp/flows/send`; setup/publish via `POST /api/whatsapp/flows/setup`. Completed forms arrive as `interactive.nfm_reply` webhooks → contact updated + chat confirmation.
