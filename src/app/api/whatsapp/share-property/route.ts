...
export async function POST(request: NextRequest) {
  try {
    ...
    // A share is a message send, not an account action: the sheet's
    // fan-out issues one call per recipient, and the 30/min admin bucket
    // refused the tail of any list longer than that.
    const limit = await checkRateLimit(
      `share-property:${ctx.userId}`,
      RATE_LIMITS.send
    );
    ...
    return NextResponse.json({
      data: {
        ...
        ...(outcome.freeformError
          ? { freeform_error: outcome.freeformError }
          : {}),
      },
    });
  } catch (err) {
