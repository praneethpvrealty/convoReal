...
describe('POST /api/whatsapp/media/upload-url', () => {
  it('[INB-013] signs one upload under this account and names its kind', async () => {
  ...
  it('[INB-013] refuses an oversized file by naming the cap it broke', async () => {
    const res = await stage({
      filename: 'tour.mp4',
      mime_type: 'video/mp4',
      size: 17 * 1024 * 1024,
  ...
  it('[INB-013] signs a video the old byte-proxying route could never carry', async () => {
    const res = await stage({
