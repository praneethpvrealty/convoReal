...
export async function POST(
  ...
  try {
    ...
    const { requester_name, requester_phone, account_id, via_contact_id, via_share_id } = body;
    ...
    const accountLimit = checkRateLimit(`locreq:account:${account_id}`, LOCREQ_ACCOUNT_LIMIT);
    ...
    if ((existingCount ?? 0) >= 3) {
      return NextResponse.json(
        { error: "You already have a pending location request for this property. Please wait for a response." },
        { status: 429 }
      );
    }
    ...
    if (typeof via_contact_id === "string" && UUID_RE.test(via_contact_id)) {
      ...
      if (
        ...
        normalizePhoneWithCountryCode(viaContact.phone || "") !== normalizedPhone
      ) {
    ...
    const { data: locRequest, error: insertErr } = await admin
      ...
      .select("id, account_id, property_id, requester_name, requester_phone, via_contact_id")
      .single();
    ...
    if (viaContactId) {
      ...
      const asked = await requestConsentFromContact(admin, locRequest, viaContactId);
      if (!asked) {
      ...
      const { data: account } = await admin
        ...
        .eq("id", account_id)
        .maybeSingle();
      const targetAgentUserId = property.user_id || account?.owner_user_id || null;
