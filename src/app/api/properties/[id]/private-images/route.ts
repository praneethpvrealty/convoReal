  ...
  try {
    ...
    if (
      ...
      throw new ForbiddenError(
        'Only admins or the listing agent can manage private photos'
      );
    }
    ...
    if (!key.startsWith(`${ctx.accountId}/`)) {
      return NextResponse.json(
        { error: 'Path outside your account' },
        { status: 403 }
      );
    }
    ...
    const sourceList: string[] = Array.isArray(
      ...
      : [];
    const entryIndex = sourceList.findIndex(
      (v) => storageObjectPath(v) === objectPath
    );
    if (entryIndex === -1) {
      return NextResponse.json(
    ...
    const nextImages = Array.isArray(property.images)
      ? [...property.images]
      : [];
    ...
    if (action === 'lock') {
      nextImages.splice(
        nextImages.findIndex(
          (v: string) => storageObjectPath(v) === objectPath
        ),
        1
      );
      if (
        !nextPrivate.some((v: string) => storageObjectPath(v) === targetPath)
      ) {
        nextPrivate.push(targetPath);
      }
    } else {
      nextPrivate.splice(
        nextPrivate.findIndex(
          (v: string) => storageObjectPath(v) === objectPath
        ),
        1
      );
      if (
        !nextImages.some((v: string) => storageObjectPath(v) === targetPath)
      ) {
