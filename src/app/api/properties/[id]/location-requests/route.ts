...
export async function PATCH(
  ...
  try {
    ...
    if (
      ...
      throw new ForbiddenError("Only admins or the listing agent can decide location requests");
    }
    ...
    const { shareLink } = await approveRequestAndSendReveal(admin, locRequest, ctx.userId);
    const { data: updated } = await ctx.supabase
      .from("property_location_requests")
      .select("*")
      .eq("id", request_id)
    ...
    return NextResponse.json({ data: maskAttributed(updated), share_link: shareLink });
  } catch (err) {
    return toErrorResponse(err);
  }
}
