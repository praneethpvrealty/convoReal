...
import { isTeaserGated, opensByDirectLink } from "@/lib/inventory/showcase-visibility";
...
export async function GET(
  ...
  try {
    ...
    let query = admin
      .from("properties")
      .select(
        "id, title, property_code, type, price, location, location_privacy, showcase_visibility, sublocality, city, state, images, account_id, is_published"
      );
    ...
    if (isTeaserGated(raw) || !opensByDirectLink(raw, false)) return new NextResponse("Not found", { status: 404 });
