...
export default function PipelinesPage() {
    ...
    if (did && !hasAutoOpenedDeal) {
      (async () => {
        const { data: deal } = await supabase
          .from('deals')
          .select(
            '*, contact:contacts(*), assignee:profiles!deals_assigned_to_fkey(*), property:properties(*)'
          )
          .eq('id', did)
          .maybeSingle();
        if (deal) {
  ...
  const [modalBrokerageType, setModalBrokerageType] = useState<
    'percentage' | 'fixed'
  >('percentage');
      ...
      >
        <DialogContent className="border-slate-700 bg-slate-900 text-slate-200 sm:max-w-md">
          ...
          <div className="space-y-4 py-3">
              ...
              <div className="space-y-1 rounded-lg border border-slate-800 bg-slate-950/40 p-3 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-slate-300">
                    {brokeragePromptDeal.title}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-450">Value:</span>
                  <span className="text-primary font-bold">
                    {formatModalDealValue(brokeragePromptDeal.value)}
                  </span>
                </div>
              </div>
            )}
            ...
            {modalBrokerageValue &&
              !isNaN(Number(modalBrokerageValue)) &&
              Number(modalBrokerageValue) > 0 && (
