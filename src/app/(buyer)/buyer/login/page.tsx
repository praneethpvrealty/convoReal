...
// ============================================================
// /buyer/login — buyer portal sign-in. Phone-first (WhatsApp OTP via
// the existing Supabase Send-SMS hook), Google secondary. Google
// users are routed through /buyer/verify-phone — a WhatsApp number is
...
export default function BuyerLoginPage() {
  ...
  useEffect(() => {
    if (otpSent) {
      setTimeout(() => document.getElementById("buyer-otp-0")?.focus(), 80);
    }
  }, [otpSent]);
  ...
  const handleOtpChange = (index: number, val: string) => {
    ...
    setOtpValues(next);
    if (digit && index < 5) document.getElementById(`buyer-otp-${index + 1}`)?.focus();
    if (next.join("").length === 6) {
      setTimeout(() => {
        (document.getElementById("buyer-otp-form") as HTMLFormElement | null)?.requestSubmit();
      }, 50);
    }
  };
  ...
  const handleOtpPaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    setOtpValues(next);
    document.getElementById(`buyer-otp-${Math.min(pasted.length, 5)}`)?.focus();
  };
  ...
  const handleSendOtp = async (e?: React.FormEvent) => {
    ...
    // app_context 'buyer' keeps first-time buyer signups out of the
    // staff account bootstrap (handle_new_user guard, migration 160).
    const { error } = await supabase.auth.signInWithOtp({
      phone: cleanPhone,
      options: { data: { app_context: "buyer" } },
    });
    if (error) {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-slate-950 px-4">
      ...
      <div className="pointer-events-none absolute right-1/4 bottom-1/4 h-[400px] w-[400px] translate-x-1/2 translate-y-1/2 rounded-full bg-emerald-500/10 blur-[100px]" />
      ...
      <Card className="relative z-10 w-full max-w-md overflow-hidden rounded-3xl border border-slate-800/80 bg-slate-900/60 p-2 shadow-2xl backdrop-blur-xl">
        ...
        <CardContent className="pt-2">
          ...
          {!otpSent ? (
            <form onSubmit={handleSendOtp} className="flex flex-col gap-4">
              <div className="flex flex-col gap-2">
                <Label htmlFor="buyer-phone" className="text-xs font-bold text-slate-300">
                  WhatsApp Number
                </Label>
            ...
            <form id="buyer-otp-form" onSubmit={handleVerifyOtp} className="flex flex-col gap-4">
              <div className="flex flex-col gap-2">
              ...
              <Button
                ...
                className="mt-1 h-10 w-full rounded-xl bg-primary text-xs font-bold text-white transition-all hover:bg-primary-hover disabled:opacity-50"
              >
                {loading ? "Verifying…" : "Verify & View My Properties"}
              </Button>
            </form>
          )}
          ...
          <p className="mt-3 text-center text-[10px] font-medium text-slate-500">
            Google sign-ins verify a WhatsApp number next — it&apos;s how we find your enquiries and
            shortlist.
          </p>
        </CardContent>
      </Card>
