...
describe("effectiveLocationPrivacy", () => {
  it("derives locality for guarded types when unset", () => {
    ...
    expect(effectiveLocationPrivacy({ type: "Villa", location_privacy: null })).toBe("locality");
  });
  ...
  it("explicit override wins over the type default", () => {
    expect(effectiveLocationPrivacy({ type: "Villa", location_privacy: "exact" })).toBe("exact");
    expect(
      effectiveLocationPrivacy({ type: "Flat/ Apartment", location_privacy: "locality" })
    ).toBe("locality");
  });

  it("ignores invalid stored values", () => {
    expect(effectiveLocationPrivacy({ type: "Villa", location_privacy: "banana" })).toBe(
      "locality"
    );
  });
});
...
describe("localityLabel", () => {
  it("joins sublocality and city", () => {
    expect(localityLabel({ sublocality: "HSR Layout", city: "Bangalore" })).toBe(
...
describe("toPublicPropertyView", () => {
  ...
  it("never leaks sensitive fields for a guarded property, even with revealExact", () => {
    const view = toPublicPropertyView(guardedProperty, { revealExact: true }) as unknown as Record<
      string,
      unknown
    >;
    for (const key of ["owner_contact_id", "notes", "deal_remarks", "documents", "private_images", "sold_price", "locality_place_id", "locality_canonical"]) {
  ...
  it("hides exact fields in buyer mode even for un-guarded properties", () => {
    ...
    const view = toPublicPropertyView(open, { revealExact: false }) as unknown as Record<
      string,
      unknown
    >;
    expect(view.location).toBe("HSR Layout, Bangalore");
  ...
  it("reveals exact fields for un-guarded properties in reveal surfaces", () => {
    const open = { ...guardedProperty, type: "Flat/ Apartment" } as Property;
    const view = toPublicPropertyView(open, { revealExact: true }) as unknown as Record<
      string,
      unknown
    >;
...
describe("canViewExactLocation", () => {
  ...
  it("anyone sees un-guarded properties", () => {
    expect(
      canViewExactLocation({ role: "viewer", userId: "u-x" }, { type: "Flat/ Apartment", user_id: "u-lister" })
    ).toBe(true);
  });

  it("admin and owner see through the guard", () => {
    expect(canViewExactLocation({ role: "admin", userId: "u-x" }, p)).toBe(true);
    expect(canViewExactLocation({ role: "owner", userId: "u-x" }, p)).toBe(true);
  });

  it("the listing agent sees their own property", () => {
    expect(canViewExactLocation({ role: "agent", userId: "u-lister" }, p)).toBe(true);
  });

  it("other agents, coordinators and viewers are blocked", () => {
    expect(canViewExactLocation({ role: "agent", userId: "u-other" }, p)).toBe(false);
    expect(canViewExactLocation({ role: "coordinator", userId: "u-other" }, p)).toBe(false);
    expect(canViewExactLocation({ role: "viewer", userId: null }, p)).toBe(false);
  });
});

describe("maskPropertyForViewer", () => {
  it("passes privileged viewers through untouched", () => {
    const out = maskPropertyForViewer(guardedProperty, { role: "owner", userId: "u-x" });
    expect(out).toBe(guardedProperty);
  });

  it("substitutes locality and strips coordinates for blocked viewers", () => {
    const out = maskPropertyForViewer(guardedProperty, { role: "agent", userId: "u-other" });
