...
describe('refuseStagedMedia', () => {
  it('[INB-013] passes an attachment staged by this account', async () => {
    expect(
      await refuseStagedMedia('acc-1', 'chat-media/acc-1/chat-1.jpeg')
    ).toBeNull();
  });

  it("[INB-013] refuses another account's path", async () => {
  ...
  it('[INB-013] refuses a path nothing was uploaded to', async () => {
    staged = null;
    const refusal = await refuseStagedMedia(
      'acc-1',
  ...
  it('[INB-013] refuses a file that landed bigger than the cap it was signed for', async () => {
