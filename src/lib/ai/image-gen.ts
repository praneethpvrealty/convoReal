...
export const IMAGE_PROVIDER_UNAVAILABLE = `AI image generation is not available on your account right now. Use "Help & feedback" at the bottom of the screen, or email ${BRANDING.supportEmail}.`;
...
export async function generateWithImagen(
  ...
  if (!response.ok) {
    ...
    throw statusError(
      errorData.error?.message ||
        `Gemini image API error: ${response.statusText}`,
      response.status
    );
  }
  ...
  if (!image?.data) {
    console.error(
      '[Gemini image] No image bytes found in response:',
...
export async function generateWithHuggingFace(
  ...
  if (imageUrl) {
    ...
    if (!imgRes.ok)
      throw statusError('Hugging Face image URL could not be fetched.', 502);
    const buffer = await imgRes.arrayBuffer();
    const ct = (imgRes.headers.get('content-type') || 'image/jpeg').split(
      ';'
    )[0];
...
export interface GenerateAiImageOptions {
  ...
  /** Abort slow providers so optional image work cannot hold the caller open. */
  timeoutMs?: number;
}
...
export async function generateAiImage(
  ...
  if (provider === 'stability') {
    if (!stabilityKey) {
      console.error(
        '[image-gen] STABILITY_API_KEY is not configured on the server.'
      );
      throw statusError(IMAGE_PROVIDER_UNAVAILABLE, 500);
    }
    try {
  ...
  try {
    if (!hfToken) {
      console.error(
        '[image-gen] HF_ACCESS_TOKEN is not configured on the server.'
      );
