...
const COMPLETED_SIGNAL =
  /\b(already (done|over|happened|visited|completed)|(is|was|has been|got) (done|completed|finished)|has visited|have visited|visited (the|him|her|them|it)|went (well|ahead|through|fine|ok|okay)|wrapped up|finished|completed it|met (him|her|them|the)|call (is )?done|site visit done|clos(?:e|ed) (?:this|it)|called(?!\s+off)(\s+(him|her|them|the|[a-z]+))?|spoke\s+(to|with)|talked\s+(to|with))\b/i;
 ...
 */
const STANDALONE_COMPLETED =
  /^(yes[, ]+)?(all |its |it'?s )?(done|completed|finished|over|visited)[\s.!👍✅]*$/i;
