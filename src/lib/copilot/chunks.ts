...
export const CHUNKS: KnowledgeChunk[] = [
  ...
  {
    id: 'deals.milestones-vs-stages',
    title: 'Milestones are not pipeline stages',
    kind: 'concept',
    body: 'The pipeline stage says where a deal sits on the board. Milestones are the closing checklist \u2014 token paid, legal verification, agreement executed, registration, handover. Completing a milestone never moves the card and moving the card never ticks a milestone, so the board and the checklist can be read independently.',
    keywords: [
      'milestone vs stage',
      'checklist',
      'kanban',
      'why did the stage not change',
    ],
  },
  {
    id: 'deals.financials-internal',
    title: 'Transaction financials stay internal',
    kind: 'concept',
    body: 'Agreed consideration, registered value, token, TDS status and instrument references are recorded on the transaction for your team only; they never appear in the public API or any shared link. When a deal closes an Owners Den bid room, the token figure comes from Token Safe and cannot be edited on the deal. The workspace records; it does not compute tax or reconcile.',
    keywords: [
      'agreed value',
      'registered value',
      'token money',
  ...
  {
    id: 'deals.share-link',
    title: 'Share a transaction with a buyer, seller or lawyer',
    kind: 'howto',
    body: 'Open the transaction, go to Stakeholders, add the person with their role and side, then tap Share link. Choose how long it lives (24 hours, 7 or 30 days) and whether it needs a one-time code, copy the link and hand it over on WhatsApp yourself. They see only what their side may see, every open is logged, and you can revoke the link any time.',
    keywords: [
      'share deal',
      'client link',
      'stakeholder',
  ...
  {
    id: 'deals.visibility',
    title: 'Who sees what on a transaction',
    kind: 'concept',
    body: 'Every note, milestone and document on a transaction has a visibility: internal only, buyer side, seller side or all stakeholders. Everything starts internal. A stakeholder link shows a person only the items marked for their side, never financials or internal notes, and photos they open carry their name as a watermark. Change a milestone or document\u2019s visibility from its row.',
    keywords: [
      'visibility',
      'internal only',
      'buyer side',
