...
export const TOURS: Tour[] = [
  {
    ...
    steps: [
      ...
      },
      {
        ...
        target: 'add-contact',
        title: 'Almost done!',
        body: 'Fill in the name and WhatsApp number, then press Save. For buyers, set **Looking to** as Buy, Rent or Either — it keeps their matches relevant. That’s it! \u{1F389}',
        advanceOn: 'next',
      },
    ],
    mobileSteps: [
  ...
  {
    id: 'set-up-showcase',
    title: 'Set up your public Showcase',
    description:
      'Choose your agency style and add services to every public listing page',
    triggers: [
      /(set ?up|edit|customi[sz]e|change).{0,25}(showcase|agency page|public page)/i,
      /(showcase|agency page|public page).{0,25}(set ?up|style|services?|articles?)/i,
      /add.{0,25}(services?|articles?).{0,25}(showcase|agency page|public page)/i,
    ],
    steps: [
      {
        ...ANYWHERE,
        target: 'nav-settings',
        title: 'Open Settings',
        body: 'Your public Showcase is configured in **Settings**. Click it at the bottom of the menu.',
        advanceOn: 'click-target',
        skipIfNextRouteActive: true,
        requiresSidebar: true,
      },
      {
        route: '/settings',
        target: 'settings-tab-showcase',
        title: 'Open Showcase',
        body: 'Click **Showcase** to edit the public page behind your property links.',
        advanceOn: 'click-target',
      },
      {
        route: '/settings',
        query: { tab: 'showcase' },
        target: 'showcase-settings-form',
        title: 'Choose your public style',
        body: 'Start with the General section: set your branding and choose the Showcase style that best fits your agency.',
        advanceOn: 'next',
      },
      {
        route: '/settings',
        query: { tab: 'showcase' },
        target: 'showcase-subtab-services',
        title: 'Add your services',
        body: 'Click **Services** to add the work your agency offers. They appear on your public Showcase page.',
        advanceOn: 'click-target',
      },
      {
        route: '/settings',
        query: { tab: 'showcase', sub: 'services' },
        target: 'showcase-services-manager',
        title: 'Build trust beyond listings',
        body: 'Add your services here. The nearby **Articles** tab works the same way for market updates and agency news.',
        advanceOn: 'next',
      },
    ],
  },
  {
    id: 'set-bot-rules',
    title: 'Set bot reply rules',
    description:
      'Give the WhatsApp bot scoped instructions for how it should reply',
    triggers: [
      /(bot|whatsapp).{0,25}(rule|instruction|behave|reply|response)/i,
      /(rule|instruction).{0,25}(bot|whatsapp|reply|response)/i,
    ],
    steps: [
      {
        ...ANYWHERE,
        target: 'nav-settings',
        title: 'Open Settings',
        body: 'Bot reply rules live in **Settings**. Click it at the bottom of the menu.',
        advanceOn: 'click-target',
        skipIfNextRouteActive: true,
        requiresSidebar: true,
      },
      {
        route: '/settings',
        target: 'settings-tab-bot-rules',
        title: 'Open Bot Rules',
        body: 'Click **Bot Rules** in the Workspace section.',
        advanceOn: 'click-target',
      },
      {
        route: '/settings',
        query: { tab: 'bot-rules' },
        target: 'bot-rule-book',
        title: 'Give the bot a focused instruction',
        body: 'Write one clear instruction, choose where it applies, then add it as active. Rules guide replies only — they never send a new message by themselves.',
        advanceOn: 'next',
      },
    ],
  },
  {
    id: 'review-conversation-gaps',
    title: 'Review conversation gaps',
    description:
      'See what may have been missed in yesterday’s WhatsApp conversations',
    triggers: [
      /(conversation|chat|whatsapp).{0,25}(gap|missed|dropped|unanswered)/i,
      /(gap|missed|dropped|unanswered).{0,25}(conversation|chat|whatsapp)/i,
    ],
    steps: [
      {
        ...ANYWHERE,
        target: 'nav-dashboard',
        title: 'Open the Dashboard',
        body: 'Your daily conversation review is on the **Dashboard**. Click it in the menu.',
        advanceOn: 'click-target',
        skipIfNextRouteActive: true,
        requiresSidebar: true,
      },
      {
        route: '/dashboard',
        target: 'dashboard-tab-gaps',
        title: 'Open Gaps',
        body: 'Click the **Gaps** tab to review what yesterday’s conversations may have left unfinished.',
        advanceOn: 'click-target',
      },
      {
        route: '/dashboard',
        query: { tab: 'gaps' },
        target: 'conversation-gaps',
        title: 'Resolve what needs attention',
        body: 'Every morning, ConvoReal reviews the previous day’s WhatsApp conversations. Open the important items first, then mark each one done or dismiss it — this screen never sends a message on its own.',
        advanceOn: 'next',
      },
    ],
  },
