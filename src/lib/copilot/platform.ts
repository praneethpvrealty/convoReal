...
export const MOBILE_APP_ROUTES: readonly string[] = [
  ...
  '/pipelines',
  '/journey',
  // Still /today, not /focus: the web route is a tab
  // (/dashboard?tab=focus) and /today is the redirect that reaches it.
  // The app maps this to its Focus screen.
  '/today',
  '/radar',
  '/pulse',
  '/agents',
...
export const MOBILE_APP_SECTION = [
  'MOBILE APP: The user is in the ConvoReal mobile app, not the web dashboard. The app covers: Inbox chats, Contacts (add, edit, import from the phone book), Properties (view, edit, share, attach and open documents, map view with a GPS "Near me" filter — ADDING a property is desktop-web only), Broadcasts, Calendar, Deals, Journey, Focus (the day’s tasks and visits, top journeys and requests to act on), Match Radar, Pulse, Team, Automations (view and pause only), Billing & AI credits, Notifications, home-screen widgets, app lock, and document/location approval cards on Home. Everything else — Settings (WhatsApp connection, templates, email lead sync, showcase), the Automations and Flows builders, Liaisons, Ads, and Requirements sharing — is desktop web only.',
