...
|---|---|---|
| Match Radar | `/dashboard?tab=radar`, `/api/radar` | Send-match, dismiss/delete, direct-owner cards. No mobile route. |
...
1. **Match Radar** — `/api/radar` already exists; list + send-match + dismiss.
2. **Todos** — `/api/todos` already exists; fold into the mobile calendar.
3. **Today** — aggregate screen over data mobile already fetches.
4. Requirements screen, deal create/edit, property creation, Den deal rooms.
5. Larger items (liaisons, buyer portal, call logs, top-up) as separate projects.
