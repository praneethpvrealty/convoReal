# मराठी (Marathi) — WhatsApp message review

These are every word ConvoReal sends to a client in Marathi. They were drafted by a machine and have **not** been checked by anyone who speaks Marathi. Please read them as a customer would.
...
Send this back with your corrections. They get applied to `src/lib/whatsapp/template-copy.ts` (the `mr:` entries), which fixes the wording for every account at once — not just one.
