# ಕನ್ನಡ (Kannada) — WhatsApp message review

These are every word ConvoReal sends to a client in Kannada. They were drafted by a machine and have **not** been checked by anyone who speaks Kannada. Please read them as a customer would.
...
Send this back with your corrections. They get applied to `src/lib/whatsapp/template-copy.ts` (the `kn:` entries), which fixes the wording for every account at once — not just one.
